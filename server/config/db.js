...
connection.query('CREATE TABLE IF NOT EXISTS users ( `id` INT UNSIGNED NOT NULL AUTO_INCREMENT , `username` VARCHAR(100) NOT NULL , `password` VARCHAR(100) NOT NULL , `created_at` DATETIME NOT NULL , PRIMARY KEY (`id`), UNIQUE (`username`)) ENGINE = InnoDB', function (error, results, fields) {
  if (error){ 
    throw error
  }
...
connection.query('CREATE TABLE IF NOT EXISTS sessions ( `id` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY , `user_id` INT UNSIGNED NOT NULL , `connected` BOOLEAN NOT NULL , `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP , `updated_at` DATETIME NULL DEFAULT NULL , FOREIGN KEY (`user_id`) REFERENCES `users`(`id`) ON DELETE CASCADE) ENGINE = InnoDB', function (error, results, fields) {
  if (error) {
    throw error
  }
  console.log('The results is: ', results);
